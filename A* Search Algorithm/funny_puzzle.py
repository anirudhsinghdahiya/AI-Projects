...
def get_succ(state):
    ...
    for empty_index in empty_indices:
        ...
        for move in moves:
            ...
            if 0 <= new_x < 3 and 0 <= new_y < 3:
                ...
                if state[new_index] == state[empty_index]:
    ...
    for state in succ_states:
        ...
        if len(find_all_indices(state, succ_states)) > 1:

            succ_states.pop(find_all_indices(state, succ_states)[0])
...
def find_all_indices(target, list_of_lists):

    indices = []

    for index, sublist in enumerate(list_of_lists):

        if sublist == target:  # Check if the current sublist matches the target
...
if __name__ == "__main__":
    ...
    get_succ([1,2,3,4,5,6,7,0,0])

    print_succ([1,2,3,4,5,6,7,0,0])